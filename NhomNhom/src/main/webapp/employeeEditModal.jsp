        ...
        <div class="bg-white rounded-2xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            ...
            <div class="bg-gradient-to-r from-amber-600 to-orange-300 p-6 rounded-t-2xl relative">
                <button onclick="closeEmployeeEditModal()"
                        class="absolute top-4 right-4 text-white hover:text-yellow-800 transition-colors">
                ...
                <p class="text-yellow-50 mt-2">Atualize os dados de um colaborador :3</p>
            </div>

            <form id="cadastroEditEmployeeForm"
                  class="p-6 space-y-4"
                  action="useremployee/update"
                  method="post">

                <input type="hidden" name="employeeId" id="employeeId">
                <div>
                    <p class="text-xs text-gray-500 font-mono ml-2">ID do Cliente: <span id="spanEmployeeId"></span></p>
                </div>
                <div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input
                                type="text"
                                name="userEditEmployeePosition"
                                id="userEditEmployeePosition"
                                required
                    ...
                    <div>
                        ...
                        <input
                                type="number"
                                name="userEditEmployeeSalary"
                                id="userEditEmployeeSalary"
                                required
                                placeholder="0.00"
                                step="0.01"
                                min="0"
                ...
                <div class="flex gap-3 pt-4 border-t border-gray-200 mt-4">
                    <button
                            type="button"
                            onclick="closeEmployeeEditModal()"
...
<script>
    const editEmployeeOverlay = document.getElementById('editEmployeeOverlay');
    const editEmployeeModalContent = document.getElementById('editEmployeeModal');

    function openEditModalEmployee(id, name, email, password, cpf, birthDate, position, salary) {

        document.getElementById('employeeId').value = id;


        document.getElementById('spanEmployeeId').innerText = id;

        document.getElementById('empEditName').value = name;
        document.getElementById('empEditEmail').value = email;
        document.getElementById('empEditPassword').value = password;
        document.getElementById('empEditBirthDate').value = birthDate;
        document.getElementById('userEditEmployeePosition').value = position;
        document.getElementById('userEditEmployeeSalary').value = salary;
        document.getElementById('empEditCpf').value = cpf;

        if (birthDate) {
            document.getElementById('empBirthDate').value = birthDate;
        }

        editEmployeeOverlay.classList.remove('hidden');
        editEmployeeModalContent.classList.remove('hidden');
    }
    ...
    if (editEmployeeOverlay) {
        editEmployeeOverlay.addEventListener('click', (e) => {
            if (e.target === editEmployeeOverlay) {
